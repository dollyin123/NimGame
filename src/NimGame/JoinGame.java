        ...
        Thread thread = new Thread() {
            ...
            public void run() {
                try {
                    ...
                    socketChannel.connect(new InetSocketAddress(new JOptionPane().showInputDialog("ip입력"), 5001)); // 192.168.0.145
                    SwingUtilities.invokeLater(()->displayText(" [접속 완료]"));
                } catch (Exception e) {
                    SwingUtilities.invokeLater(()->displayText(" [접속 실패]"));
        ...
        if(turn == 1) {
            ...
            switch(data) {
                ...
                case Data.GIVEUP:
                    ...
                    break;
                default :
                    if(Long.parseLong(data) > 10000000000L) {
                        Beads.line[0] = Integer.parseInt(data.substring(0,2));
                        Beads.line[1] = Integer.parseInt(data.substring(2,4));
                        Beads.line[2] = Integer.parseInt(data.substring(4,6));
                        Beads.line[3] = Integer.parseInt(data.substring(6,8));
                        Beads.line[4] = Integer.parseInt(data.substring(8,10));
                        Beads.line[5] = Integer.parseInt(data.substring(10,12));
                        send(Data.GAMESTART);
                    }
            }
        }
